...
namespace backend.Core.Services
{
    public class MessageService : IMessageService
    {
        ...
        public MessageService(ApplicationDbContext context, ILogServices logServices, UserManager<ApplicationUser> userManager)
        {
            _context = context;
        ...
        public async Task<GeneralServiceResponseDto> CreateNewMessageAsync(ClaimsPrincipal User, CreateMessageDto createMessageDto)
        {
            if(User.Identity.Name == createMessageDto.ReceiverUserName)
                return new GeneralServiceResponseDto()
                {
                    IsSucceed = false,
                    StatusCode = 400,
                    Message = "Sender and Recevier can not be the same",
                };
            ...
            if (!isReceiverUserNameValid)
                return new GeneralServiceResponseDto()
                {
                    IsSucceed = false,
                    StatusCode = 400,
                    Message = "Recevier UserName is not valid",
                };
