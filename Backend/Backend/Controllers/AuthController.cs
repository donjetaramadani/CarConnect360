...
namespace backend.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            var seedResult = await _authServices.SeedRolesAsync();
        ...
        {
            var updateRoleResult = await _authServices.UpdateRoleAsync(User, updateRoleDto);
        ...
        public async Task<ActionResult<LoginServiceResponseDto>> Me([FromBody] MeDto token)
        {
            try
            {
                var me = await _authServices.MeAsync(token);
                if (me is not null)
                {
                    return Ok(me);
                }
        ...
        {
            var usersList = await _authServices.GetUsersListAsync();
        ...
        {
            var user = await _authServices.GetUserDetailsByUserNameAsync(userName);
            if (user is not null)
            {
                return Ok(user);
            }
        ...
        {
            var usernames = await _authServices.GetUsernamesListAsync();
